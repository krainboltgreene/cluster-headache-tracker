defmodule CoreWeb.AccountAuthTest do
  use CoreWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Core.Users
  alias CoreWeb.AccountAuth
  ...
  describe "log_in_account/3" do
    test "stores the account token in the session", %{conn: conn, account: account} do
      ...
      assert Users.get_account_by_session_token(token)
    end
  ...
  describe "logout_account/1" do
    test "erases session and cookies", %{conn: conn, account: account} do
      account_token = Users.generate_account_session_token(account)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Users.get_account_by_session_token(account_token)
    end
  ...
  describe "on_mount: mount_current_account" do
    test "assigns current_account based on a valid account_token ", %{
      conn: conn,
      account: account
    } do
      account_token = Users.generate_account_session_token(account)
    ...
    test "assigns nil to current_ account assign if there isn't a valid account_token ", %{
      conn: conn
    } do
    ...
    test "assigns nil to current_ account assign if there isn't a account_token", %{conn: conn} do
